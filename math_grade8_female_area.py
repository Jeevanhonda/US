...
def show():
    ...
    st.header("Math-Grade 8 Gender-wise Average Scale Score Visualization")

    @st.cache_data
    def load_data():
        try:
            df = pd.read_csv("grade_8_prediction_result.csv")
            return df
    ...
    def process_gender_data(gender):
        ...
        df_combined = pd.concat([df_melted_actual, df_melted_predicted])
    ...
    def plot_chart(df_combined, chart_type, gender):
        ...
        min_val = df_combined["Average Scale Score"].min()  # Get the minimum value in the dataset
        ...
        # Apply y-axis range globally
        fig.update_layout(
                        yaxis=dict(
                            range=[df_combined["Average Scale Score"].min() -250, df_combined["Average Scale Score"].max() + 350]
                        )
                    )
    ...
    # Display Chart
    plot_chart(df_combined, "Area", "Female")
